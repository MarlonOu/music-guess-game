 ...
 *
 * 優先序：Apple Music → Deezer → YouTube。前兩者都用同源的原生 <audio> 元素播放，
 ...
 * 各自會有對方沒有的冷門/地區限定歌曲），只有兩者都沒有時才退回用 YouTube。
 *
 * 已知取捨：Apple／Deezer 官方試聽都是「固定的一段」（通常從歌曲某個固定秒數開始、長度固定
 * 在 30 秒上下），不像 YouTube 完整影片可以任意指定開始秒數。因此用這兩個來源播放時，一律從
 * 試聽片段本身的開頭（0 秒）開始，不套用原本針對「完整歌曲」計算出來的 clipStartSec——那個
 * 數字是相對於完整歌曲長度算的隨機片段起點，套在只有 30 秒的試聽片段上沒有意義，甚至可能
 * 超出片段本身的長度。這代表 RANDOM_CLIP 模式用這兩個來源播放時，實際上會是「試聽片段的
 * 前幾秒」而非真正隨機的片段——這是目前的已知限制，如果想保留隨機性，需要額外記錄每個試聽
 * 片段自己的實際長度，之後可以再優化，屬於後續加強項目。
 *
 * 回傳 null 代表這首歌三種來源都沒有（理論上不該發生，後台表單與 CSV 匯入都要求至少一種
...
export function resolvePlaybackTarget(song: PlayableSong, question: PlayableQuestion): PlaybackTarget | null {
  ...
  if (nativeAudioUrl && nativeSource) {
    ...
    return { source: nativeSource, idOrUrl: nativeAudioUrl, startSec: 0, durationSec };
  }
